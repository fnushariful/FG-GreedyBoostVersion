...
void generatePointsInsideADisc(const unsigned n, const double radiusOfDisk, vector<Point> &P) {
  vector<K::Point_2> points;
  typedef Random_points_in_disc_2< K::Point_2, Creator_uniform_2< double, K::Point_2 > > Point_generator;
...
void generatePointsInsideASquareNormal(const unsigned pointsInACuster, const unsigned numberOfClusters, vector<Point> &P) {
  vector<K::Point_2> points;
  ...
  std::normal_distribution<double> distributionX(2.0,2.0), distributionY( 2.0,2.0);
...
void generateContiguousPointsOnAGrid(const unsigned n, vector<Point> &P) {
  vector<Point> points;
  points_on_square_grid_2( 10, n, std::back_inserter(points), Creator_uniform_2<int,Point>());
  // perturb_points_2( points.begin(), points.end(), 0.0001, 0.0001 );
...
void generateRandomInsideAnAnnulus(const unsigned n, const double r2, const double r1, vector<Point> &P) {
